...
class Repressor:
    def __init__(self, name, type, color, promoter, promoter_dna, ribozyme, ribozyme_dna, rbs, rbs_dna, cds, cds_dna, terminator, terminator_dna, ymax, ymin, k, n, il, ih, equation):
        self.name = name
        self.type = type
        self.color = color
...
if __name__ == '__main__':
    ctx = CtxObject()
    # repressor_list = get_netlist(ctx, "j4", "resources/Inputs.txt", "resources/Outputs.txt", "resources/0xFE.v", "JSON.UCF.json -plasmid false -eugene false")
    repressor_dict = parse_csv_file("resources/gates_Eco1C1G1T1.csv")
    write_back_to_csv(repressor_dict)
    # improve_scores(repressor_dict, repressor_list)
